...
function renderList(listEl, items, type) {
  ...
  items.forEach(word => {
    ...
    if (type === 'favorites') { li.querySelector('.remove').addEventListener('click', () => { const favs = loadJSON(STORAGE_FAVORITES, []); saveJSON(STORAGE_FAVORITES, favs.filter(w => w !== word)); renderLists(); showToast(`Removed “${word}” from favorites`, 'warn'); }); }
    listEl.appendChild(li);
  });
}
function renderLists(){ const recent = loadJSON(STORAGE_RECENT, []); const favs = loadJSON(STORAGE_FAVORITES, []); renderList(recentList, recent, 'recent'); renderList(favoritesList, favs, 'favorites'); }
...
if (exportFavoritesBtn) exportFavoritesBtn.addEventListener('click', () => { const favs = loadJSON(STORAGE_FAVORITES, []); const blob = new Blob([JSON.stringify({ favorites: favs, exported: new Date().toISOString().slice(0,10) }, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `favorites-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url); showToast('Favorites exported','success'); });
...
function buildEnhancedCard(wordsData) {
  ...
  let examplesHTML = '';
  if (examples.length) { examplesHTML = `<ol class=\"examples-list\">${examples.slice(0,6).map(e => `<li><strong>${e.pos || primaryPOS}</strong> — ${e.ex}</li>`).join('')}</ol>`; }
  else {
    const auto = [];
  ...
  favBtn.addEventListener('click', () => { let favs = loadJSON(STORAGE_FAVORITES, []); const nowFav = favBtn.classList.toggle('active'); favBtn.setAttribute('aria-pressed', String(nowFav)); if (nowFav) { if (!favs.includes(title)) favs.push(title); favBtn.title = 'Remove from favorites'; showToast(`Added “${title}” to favorites`, 'success'); } else { favs = favs.filter(w => w !== title); favBtn.title = 'Add to favorites'; showToast(`Removed “${title}” from favorites`, 'warn'); } saveJSON(STORAGE_FAVORITES, favs); renderLists(); });
  card.querySelector('#shareBtn').addEventListener('click', async () => { const text = `${title}\n${card.querySelector('[data-panel="defs"]').innerText}`; if (navigator.share) { try { await navigator.share({ title: `Dictionary: ${title}`, text }); showToast('Shared', 'success'); } catch {} } else { await navigator.clipboard.writeText(text); showToast('Copied to clipboard', 'success'); } });
  card.querySelector('#copyBtn').addEventListener('click', async () => { const text = `${title}\n${card.querySelector('[data-panel="defs"]').innerText}`; await navigator.clipboard.writeText(text); showToast('Copied to clipboard', 'success'); });
...
(function initTheme(){ const saved = localStorage.getItem(STORAGE_THEME) || 'dark'; document.documentElement.classList.toggle('light', saved === 'light'); if (themeToggle) themeToggle.innerHTML = `<i class=\"fa-solid ${saved === 'light' ? 'fa-sun' : 'fa-moon'}\"></i>`; })();
if (themeToggle) { themeToggle.addEventListener('click', () => { const isLight = document.documentElement.classList.toggle('light'); localStorage.setItem(STORAGE_THEME, isLight ? 'light' : 'dark'); themeToggle.innerHTML = `<i class=\"fa-solid ${isLight ? 'fa-sun' : 'fa-moon'}\"></i>`; showToast(isLight ? 'Light theme' : 'Dark theme', 'info'); }); }

(function initWODMini(){ if (!wordOfDayBtn || !quickPills) return; const idx = (new Date().getUTCDate() + new Date().getUTCMonth()) % SEED_WORDS.length; const wod = SEED_WORDS[idx]; wordOfDayBtn.textContent = `Word of the day: ${wod}`; wordOfDayBtn.addEventListener('click', () => { query.value = wod; fetchword(); }); quickPills.innerHTML = SEED_WORDS.slice(0,6).map(w => `<button class=\"pill\">${w}</button>`).join(''); [...quickPills.querySelectorAll('.pill')].forEach(btn => btn.addEventListener('click', () => { query.value = btn.textContent; fetchword(); })); })();

function wodIndex(seed=SEED_WORDS) { const d = new Date(); return (d.getUTCDate() + d.getUTCMonth()) % seed.length; }
async function fetchWODData(word) { try { const resp = await fetch(`${api}${encodeURIComponent(word)}`); if (!resp.ok) throw new Error(resp.status); const json = await resp.json(); const w0 = json[0] || {}; const firstIpa = (w0.phonetics || []).find(p => p.text)?.text || ''; const firstAudio = (w0.phonetics || []).find(p => p.audio)?.audio || ''; let shortDef = ''; let example = ''; (w0.meanings || []).forEach(m => { (m.definitions || []).forEach(d => { if (!shortDef && d.definition) shortDef = d.definition; if (!example && d.example) example = d.example; }); }); return { word: w0.word || word, ipa: firstIpa, audio: firstAudio, def: shortDef, example }; } catch { return { word, ipa: '', audio: '', def: 'Explore the meaning by clicking Learn more.', example: '' }; } }
async function renderWOD(word) { const data = await fetchWODData(word); if (wodWordEl) wodWordEl.textContent = data.word || word; if (wodIpaEl) { wodIpaEl.textContent = data.ipa || '/'; wodIpaEl.disabled = !data.ipa; } if (wodAudioBtn) { wodAudioBtn.disabled = !data.audio; wodAudioBtn.onclick = () => { if (data.audio) playAudio(data.audio); }; } if (wodMeaningEl) wodMeaningEl.textContent = truncate(data.def || 'No definition available.'); if (wodExampleEl) wodExampleEl.textContent = data.example ? `Example: ${data.example}` : ''; if (wodLearnBtn) { wodLearnBtn.onclick = () => { const q = encodeURIComponent(data.word || word); window.open(`https://www.google.com/search?q=${q}`, '_blank', 'noopener,noreferrer'); }; } if (wordOfDayBtn) wordOfDayBtn.textContent = `Word of the day: ${data.word}`; }
function setWODCollapsed(collapsed) { if (!wodCard) return; wodCard.classList.toggle('wod-collapsed', collapsed); localStorage.setItem(STORAGE_WOD_COLLAPSED, collapsed ? '1' : '0'); }
(function initWODHero(){ if (!wodCard) return; const idx = wodIndex(); let currentWord = SEED_WORDS[idx]; const collapsed = localStorage.getItem(STORAGE_WOD_COLLAPSED) === '1'; setWODCollapsed(collapsed); renderWOD(currentWord); if (wodRefreshBtn) wodRefreshBtn.addEventListener('click', async () => { const i = (SEED_WORDS.indexOf(currentWord) + 1) % SEED_WORDS.length; currentWord = SEED_WORDS[i]; await renderWOD(currentWord); }); if (wodCollapseBtn) wodCollapseBtn.addEventListener('click', () => { const nowCollapsed = !wodCard.classList.contains('wod-collapsed'); setWODCollapsed(nowCollapsed); wodCollapseBtn.innerHTML = `<i class=\"fa-solid ${nowCollapsed ? 'fa-chevron-down' : 'fa-chevron-up'}\"></i>`; }); })();
